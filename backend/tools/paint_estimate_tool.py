...
def paint_estimate_logic(
    ...
    try:
        number_of_rooms = shared_inputs.get("number_of_rooms", 1)
        number_of_bathrooms = shared_inputs.get("number_of_bathrooms", 1)
        number_of_kitchens = shared_inputs.get("number_of_kitchens", 1)

        room_list = parse_sizes(room_sizes)
        bath_list = parse_sizes(bathroom_sizes)
        kitchen_list = parse_sizes(kitchen_sizes)

        # Pad lists to match declared counts
        if not room_list:
