...
def gray_structure_logic(
    ...
    number_of_bathrooms = shared_inputs.get("number_of_bathrooms") or number_of_rooms
    number_of_kitchens = shared_inputs.get("number_of_kitchens") or 1

    if not room_list:
        room_list = [(12, 12)] * number_of_rooms
    elif len(room_list) < number_of_rooms:
        room_list += [room_list[-1]] * (number_of_rooms - len(room_list))

    if not bath_list:
        bath_list = [(6, 6)] * number_of_bathrooms
    elif len(bath_list) < number_of_bathrooms:
        bath_list += [bath_list[-1]] * (number_of_bathrooms - len(bath_list))

    if not kitchen_list:
        kitchen_list = [(8, 10)] * number_of_kitchens
    elif len(kitchen_list) < number_of_kitchens:
        kitchen_list += [kitchen_list[-1]] * (number_of_kitchens - len(kitchen_list))
    ...
    plaster_volume = (total_wall_area * 2 + total_ceiling_area) * 0.042  # walls: both sides, ceiling: one side
