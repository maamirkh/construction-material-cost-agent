...
def concrete_logic(
    ...
    try:
        ...
        include_tower = shared_inputs.get("include_tower", False)
        tower_length_ft = shared_inputs.get("tower_length_ft", 0.0)
        tower_width_ft = shared_inputs.get("tower_width_ft", 0.0)

        tower_col_volume = tower_slab_volume = tower_beam_volume = 0.0
        if include_tower:
            tower_col_volume = 4 * (1.5 * 0.5 * 10)
            tower_slab_volume = (tower_length_ft or 6) * (tower_width_ft or 6) * 0.42
            tower_beam_volume = ((tower_length_ft or 6) + (tower_width_ft or 6)) * 2 * 1.5 * 0.5
